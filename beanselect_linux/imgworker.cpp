...
#ifdef unix
    if(NCS==NULL){
        NCS=new NCSWrapper(NETWORK_INPUT_SIZE, NETWORK_OUTPUT_SIZE,3,true,false);
    }
    if (!NCS->load_file(graph_file.c_str()))
        ncsinited=false;
    else
        ncsinited=true;
    Mat meanImg=imread(mean_file);
    NCS->setmean(meanImg);
...
//    }
...
#elif unix
    NCS->process(classifymat,resized16f);
    //加载图片
    ...
    //std::count<<"unix"<<std::endl;
#elif RASPI
    //std::count<<"ras"<<std::endl;
#endif
}
